...
int main(void) {
    ...
    if ( cur_init(&app,1) == CUR_OK) {
        ...
        db_mysql(&app,"localhost","root","root");
        db_connect( &app );
        db_exec(&app,"CREATE DATABASE IF NOT EXISTS curmudgeon");
        db_select_db(&app,"curmudgeon");
        db_exec(&app,"CREATE TABLE IF NOT EXISTS people (first_name VARCHAR(55), last_name VARCHAR(55))");
        ...
        db_query(&app,"SELECT * FROM people");
        db_row_t * row = NULL;
        while ( db_next(app,&row) == CUR_OK ) {
            int ri;
        ...
        json_t * jrow = NULL;
        db_row_t * row2 = NULL;
        while ( db_next_as_json(app,&jrow,&row2) == CUR_OK ) {
        ...
        db_result_as_json(&app,&all_rows);
        char * rows_string = json_dumps(all_rows,0);
        //printf("%s\n",rows_string);
        free(rows_string);

        json_t * find_test = NULL; 
        db_find_by(&app,&find_test,"people","first_name","jason",NULL);
        //printf("%s\n",json_dumps(find_test,0));
        json_t * find_sql = NULL;
        db_find_by_sql(&app,&find_sql,"SELECT * FROM %s WHERE %s = %s","people","first_name","'jason'",NULL);
